...
def bce_rescale_loss(config, logits_text, logits_visual, logits_iou, iou_mask_map, gt_maps, gt_times, word_label, word_mask,loss_itc,writer=None):
    #print((logits_text.shape,logits_visual.shape,logits_iou.shape,iou_mask_map.shape))
    #print((gt_maps.shape,gt_times.shape,word_label.shape,word_mask.shape))
    T = gt_maps.shape[-1]
    ...
    ##grid=(4*no_of_words*1514) and word_label=(4*no_of_words*1)
    loss_entropy = loss.sum(-1).mean()
    loss_reg = loss_reg.mean()
    loss_iou = loss_iou.mean()
    #print([float(loss_entropy),float(loss_reg),float(loss_iou), float(text_loss),float(loss_itc)])
    loss_value = config.W1*loss_entropy  + config.W2* loss_reg + config.W3*loss_iou + config.W4*text_loss + config.W5*loss_itc
    ##writer.writerow([float(loss_entropy),float(loss_reg),float(loss_iou), float(text_loss),float(loss_value),float(loss_itc)])
